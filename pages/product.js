...
const fs = require('fs');
 ...
 class ProductPage extends BasePage {
    ...
    //  }
      async clickDigitaltOption() {
        const digitalOption = await $("//a[contains(@class,'dropdown-item') and .//img[contains(@src, 'digital_product.svg')]]");
        await digitalOption.waitForDisplayed({ timeout: 10000 });
  ...
  async clickSaveProductButton() {
    const saveButton = await $('button=Save product'); // Modify selector if needed
    await saveButton.waitForClickable({ timeout: 10000 });
    await saveButton.click();
  }

  async clickProductSidebarBtn() {
    const productSidebarBtn = await $('a=Products'); // Adjust based on your app
    await productSidebarBtn.waitForClickable({ timeout: 10000 });
    await productSidebarBtn.click();
  }

  async clickAllProductsLink() {
    const allProductsLink = await $('a=All Products'); // Adjust selector
    await allProductsLink.waitForClickable({ timeout: 10000 });
...
async uploadImage() {
    ...
    const imagePath = path.resolve(
        process.cwd(),
        'features/imageupload/PdixZ1DrHAsICbZzHlMF3e60b40c2c9d28.jpg'
    );
    ...
    await browser.executeAsync((base64, done) => {
        ...
        const file = new File(
            [byteArray],
            'PdixZ1DrHAsICbZzHlMF3e60b40c2c9d28.jpg',
            { type: 'image/jpg' }
        );
